...
Type objective_function<Type>::operator() ()
{
  ...
  DATA_ARRAY(Omega_Y);    //inverse of Sigma_Y (log scale)

  // normalization flag - used for speed -up
  DATA_INTEGER(flag); // flag == 0 => no data contribution added to jnll
  
  // Indices
  DATA_INTEGER( n_i ); // Number of data points in space; assumed same for each surface for now 
  ...
  matrix<Type> Expected(n_s,n_surf);
