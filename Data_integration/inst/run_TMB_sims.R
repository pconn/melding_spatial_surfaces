...
RMSE = array(NA,dim=c(length(N_surfaces),length(Unbiased),7,n_sims)) #next to last array dim is for estimator type
  ...
  for(iscen in 1:length(N_surfaces)){
    ...
    for(ibias in 1:length(Unbiased)){
      ...
      if(det_flag==0){  #only run if all surfaces have pos def VC matrix
        ...
        #Devin's constrained model with Mu set to zero; fit individual surfaces then take mean of Xi
        Xi_mat = matrix(0,n_surf,n_knots)
        Data_new = Data_all
        Params_new = Params_all
        Params_new$Xi_s = matrix(0,n_knots,1)
        Params_new$log_kappa_xi = Params_new$log_tau_xi = Params_new$log_alpha = 0
        Data_new$n_surf=1
        Omega_Y = Data_all$Omega_Y
        Data_new$Omega_Y = array(dim=c(n_s,n_s,1))
        Map_all = list("Mu_s"=factor(matrix(NA,n_knots,1)),"log_kappa_mu"=factor(NA),
                       "log_tau_mu"=factor(NA),"log_alpha"=factor(NA))
        
        for(isurf2 in 1:n_surf){
          Data_new$Y_i = matrix(Y_ind[[isurf2]]-mean(Y_ind[[isurf2]]),n_s,1)
          Data_new$Omega_Y[,,1] = Omega_Y[,,isurf2]
          Obj = MakeADFun( data=Data_new, parameters=Params_new, random=Random_all, map=Map_all, DLL="fit_multiple_surfaces",silent=FALSE)
          Obj$fn( Obj$par )
          Opt = nlminb( start=Obj$par, objective=Obj$fn, gradient=Obj$gr, lower=Lower, upper=Upper, control=list(trace=1, eval.max=500, iter.max=500))         #
          Report = Obj$report()
          Xi_mat[isurf,] = Report$Xi
        }
        
        Xi_mean = colMeans(Xi_mat)
        Pi_est = exp(Xi_mean[1:n_s])/sum(exp(Xi_mean[1:n_s]))
        
        RMSE[iscen,ibias,7,isim] = sqrt(sum((Pi-Pi_est)^2)/n_s)
        
      }
    }
  }
  save(RMSE,file="sim_RMSE_add.RData")
